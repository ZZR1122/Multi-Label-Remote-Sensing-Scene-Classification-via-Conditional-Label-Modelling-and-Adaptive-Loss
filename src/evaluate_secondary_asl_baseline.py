...
from dinov3_secondary_baselines import DINOv3ASLClassifier
from train_secondary_asl_baseline import SecondaryLabelDataset, load_split


# Loads the trained ASL baseline model for evaluation.
def build_model(num_secondary_labels, device, model_path):
    """Load the trained ASL baseline and switch it to eval mode."""

    model = DINOv3ASLClassifier(num_classes=num_secondary_labels).to(device)
...
# Evaluates the ASL baseline model and writes CSV reports.
def test_model(
    model_path,
    split_file,
    report_dir,
    ...
    """Run ASL baseline evaluation and export summary, per-class metrics, and hard cases."""
    ...
    summary = {
        ...
        "eval_loss_name": "AsymmetricLossOptimized",
        "avg_asl_loss": float(avg_loss),
        "experiment_name": "dinov3_asl_independent",
        "backbone": "DINOv3",
        "loss": "ASL",
...
if __name__ == "__main__":
    main(
        model_path="ablation_models/dinov3_asl_independent/best_model_multilabel_asl_independent.pth",
        split_file="data_splits/test.json",
        report_dir="results/eval_report_multilabel_asl_independent",
        batch_size=32,
        num_workers=4,
        threshold=0.5,
    )
