...
from dinov3_secondary_baselines import DINOv3ASLGBClassifier
from train_secondary_aslgb_baseline import SecondaryLabelDataset, load_split


# Loads the trained ASL-GB baseline model for evaluation.
def build_model(num_secondary_labels, device, model_path):
    """Load the trained ASL-GB baseline and switch it to eval mode."""

    model = DINOv3ASLGBClassifier(num_classes=num_secondary_labels).to(device)
...
# Evaluates the ASL-GB baseline model and writes CSV reports.
def test_model(
    model_path,
    split_file,
    report_dir,
    ...
    """Run ASL-GB baseline evaluation and export summary, per-class metrics, and hard cases."""
    ...
    criterion = multilabel_losses.AsymmetricGBLoss()
    ...
    summary = {
        ...
        "eval_loss_name": "AsymmetricGBLoss",
        "avg_aslgb_loss": float(avg_loss),
        "experiment_name": "dinov3_aslgb_independent",
        "backbone": "DINOv3",
        "loss": "ASL-GB",
...
if __name__ == "__main__":
    main(
        model_path="ablation_models/dinov3_aslgb_independent/best_model_multilabel_aslgb_independent.pth",
        split_file="data_splits/test.json",
        report_dir="results/eval_report_multilabel_aslgb_independent",
        batch_size=32,
        num_workers=4,
        threshold=0.5,
    )
