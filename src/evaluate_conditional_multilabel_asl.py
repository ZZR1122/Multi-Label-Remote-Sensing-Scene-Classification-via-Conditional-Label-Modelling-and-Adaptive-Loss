...
from train_conditional_multilabel_asl import PrimarySecondaryLabelDataset
...
# Loads the trained conditional ASL model for evaluation.
def build_model(num_secondary_labels, num_primary_labels, device, model_path):
    """Load the trained conditional multilabel checkpoint and switch it to eval mode."""
...
# Evaluates the conditional ASL model and writes CSV reports.
def test_model(
    model_path,
    split_file,
    report_dir,
    ...
    """Run multilabel evaluation and export summary, per-class metrics, curves, and hard cases."""
    ...
    # Threshold the sigmoid outputs once so every downstream metric uses the same
    ...
    # This table records the behavior of every secondary label independently.
    rows = []
    for c in range(num_secondary_labels):
        rows.append(
            {
...
if __name__ == "__main__":
    main(
        model_path="model/best_model_conditional_multilabel_asl.pth",
        split_file="data_splits/test.json",
        report_dir="results/eval_report_conditional_multilabel_asl",
        batch_size=32,
        num_workers=4,
        threshold=0.5,
    )
