...
# Loads the trained conditional ASL-GB model for evaluation.
...
# Evaluates the conditional ASL-GB model and writes CSV reports.
def test_model(
    model_path,
    split_file,
    report_dir,
...
if __name__ == "__main__":
    main(
        model_path="model/best_model_conditional_multilabel_aslgb.pth",
        split_file="data_splits/test.json",
        report_dir="results/eval_report_conditional_multilabel_aslgb",
        batch_size=32,
        num_workers=4,
        threshold=0.5,
    )
