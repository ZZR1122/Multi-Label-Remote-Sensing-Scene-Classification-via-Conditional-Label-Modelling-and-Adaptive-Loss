...
from dinov3_secondary_baselines import DINOv3BCECorrelationClassifier
from train_conditional_bce_correlation import PrimarySecondaryLabelDataset, load_split


# Loads the trained conditional BCE model for evaluation.
def build_model(num_secondary_labels, num_primary_labels, device, model_path):
    """Load the trained conditional BCE baseline and switch it to eval mode."""

    model = DINOv3BCECorrelationClassifier(
...
# Evaluates the conditional BCE model and writes CSV reports.
def test_model(
    model_path,
    split_file,
    report_dir,
    ...
    """Run conditional BCE evaluation and export summary, per-class metrics, and hard cases."""
    ...
    model = build_model(num_secondary_labels, num_primary_labels, device, model_path)
    ...
    total_loss = 0.0
    with torch.no_grad():
        for images, primary_label_targets, secondary_label_targets in tqdm(test_loader, desc="Evaluating"):
    ...
    summary = {
        ...
        "experiment_name": "dinov3_bce_correlation",
        "backbone": "DINOv3",
        "loss": "BCEWithLogitsLoss",
        "correlation_modeling": 1,
        "uses_primary_condition": 1,
    }
...
if __name__ == "__main__":
    main(
        model_path="ablation_models/dinov3_bce_correlation/best_model_multilabel_bce_correlation.pth",
        split_file="data_splits/test.json",
        report_dir="results/eval_report_multilabel_bce_correlation",
        batch_size=32,
        num_workers=4,
        threshold=0.5,
    )
